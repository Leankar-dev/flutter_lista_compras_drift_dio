...
class _ProductsScreenState extends State<ProductsScreen> {
  ...
  void initState() {
    ...
    productsDriftHandler
        .initialize(widget.listin.id, appDatabase)
        .then((value) => refresh());
    super.initState();
  }
  ...
  refresh() async {
    List<Product> listProducts = await productsDriftHandler.getProducts();

    _filterProducts(listProducts);
  }

  _removeProduct(Product produto) async {
    await productsDriftHandler.removeProduct(produto);
    refresh();
  }
  ...
  showAddEditModal({Product? product}) {
    showProductAddEditProductModal(
      ...
      onRefresh: refresh,
      product: product,
      productsDriftHandler: productsDriftHandler,
    );
  }

  _toggleProductStatus({
    required Product produto,
    required String listinId,
  }) async {
    produto.isPurchased = !produto.isPurchased;
    await productsDriftHandler.updateProduct(produto);
    refresh();
  }
